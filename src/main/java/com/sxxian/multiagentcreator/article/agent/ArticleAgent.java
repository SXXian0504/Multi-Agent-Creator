package com.sxxian.multiagentcreator.article.agent;
...
public class ArticleAgent {
    ...
    public ArticleContext generateTitles(ArticleContext context) {
        String prompt = PromptConstant.AGENT1_TITLE_PROMPT
                ...
                + buildRetrievedContextPrompt(context)
    ...
    public Map<String, Object> generateTitlesNode(OverAllState state) {
        ...
        context.setRetrievedContext(readString(state, "retrievedContext", null));
    ...
    private String buildRetrievedContextPrompt(ArticleContext context) {
        if (context.getRetrievedContext() == null || context.getRetrievedContext().isBlank()) {
            return "";
        }
        return "\n\n请参考以下用户知识库资料。资料用于约束事实、案例、设定和个人表达风格；不要逐字照抄，"
                + "也不要编造资料中不存在的具体数据。\n"
